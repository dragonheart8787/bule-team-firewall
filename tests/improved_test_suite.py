...
async def run_improved_tests():
    ...
    results = {
        ...
        "system_info": {
            "waf_version": "Enterprise Fixed v1.1",
    ...
    test_cases = [
        ...
        {"url": "http://localhost:8080/administrator", "expected": 403, "name": "管理員路徑變體"},
    ...
    try:
        async with aiohttp.ClientSession() as session:
            for i in range(30):  # 減少到 30 個請求以加快測試
                try:
                    request_start = time.time()
                    async with session.get("http://localhost:8080/", timeout=aiohttp.ClientTimeout(total=2)) as response:
    ...
    if response_times:
        ...
        # 調整性能標準：成功率 > 95%, 平均響應時間 < 500ms
        performance_passed = success_rate > 95 and avg_response_time < 500
    else:
        performance_passed = False
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get("http://localhost:8001/status", timeout=aiohttp.ClientTimeout(total=5)) as response:
                if response.status == 200:
                    ...
                    print(f"  [OK] SIEM 狀態正常: {data.get('status', 'unknown')}")
                else:
    ...
    # 保存報告
    report_file = f"IMPROVED_TEST_REPORT_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    with open(report_file, 'w', encoding='utf-8') as f:
        json.dump(results, f, indent=2, ensure_ascii=False)
    
    print("="*80)
    print("企業級 WAF 系統改進測試報告")
    print("="*80)
